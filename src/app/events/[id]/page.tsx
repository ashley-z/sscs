...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="flex-1 bg-cream pb-16">
      {/* Header Banner */}
      <div className="relative h-[300px] md:h-[400px] w-full bg-gradient-to-br from-charcoal to-primary-dark">
        <div className="absolute inset-0 bg-charcoal/40 flex flex-col justify-end pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          <span className="bg-primary px-3 py-1 text-xs font-bold uppercase tracking-wider text-white rounded-full w-fit mb-4">
            {event.category}
          </span>
          <h1 className="font-display text-3xl md:text-5xl font-bold text-white mb-2 max-w-3xl leading-tight">
            {event.title}
          </h1>
          <p className="text-white/80 font-medium text-lg">{fullDate}</p>
        </div>
      </div>
